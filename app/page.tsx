...
function HeroLogo() {
  return (
    <div className="flex items-center justify-center">
      <Image
        src="/images/logo.png"
        alt="World Humanity Inc. globe logo"
        width={400}
        height={400}
        className="w-64 sm:w-80 lg:w-96 drop-shadow-lg"
        priority
      />
    </div>
  );
        ...
        <div className="mx-auto grid max-w-page items-center gap-10 px-4 pb-16 pt-14 sm:px-6 lg:grid-cols-[1.1fr_1fr] lg:pb-24 lg:pt-20">
          <div>
            <div className="rise-in">
              <Eyebrow tone="marigold">100% volunteer-run since day one</Eyebrow>
            </div>
            <h1 className="rise-in rise-in-delay-1 mt-5 font-display text-4xl font-extrabold leading-[1.05] text-ink sm:text-5xl lg:text-6xl">
              Well-being for{" "}
              <span className="text-ocean">all people,</span>{" "}
            ...
            <p className="rise-in rise-in-delay-2 mt-6 max-w-xl text-lg leading-relaxed text-ink/70">
              {org.shortName} is a coalition of neighbors who show up — hosting blood drives with
              Carter BloodCare, packing meals for families, and walking for hearts with the
              American Heart Association. No paid staff. Just people helping people.
            </p>
          ...
          <div className="ticker-track">
            {[...tickerItems, ...tickerItems].map((item, i) => (
              <span key={i} className="flex items-center whitespace-nowrap px-6 text-sm font-semibold text-ink/60">
                <span className="mr-6 h-1.5 w-1.5 rounded-full bg-marigold" />
                {item}
              </span>
            ))}
          </div>
            ...
            {programs.map((p) => {
              ...
              return (
                <Link key={p.slug} href={`/our-work#${p.slug}`} className="group relative overflow-hidden rounded-2xl bg-white p-7 shadow-sm ring-1 ring-ink/5 transition-all hover:-translate-y-1 hover:shadow-lg">
                  ...
                  <span className="mt-4 inline-flex items-center gap-1.5 font-semibold text-ocean">
                    Learn more
